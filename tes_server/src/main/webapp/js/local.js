...
function register_nav() {
	$(".nav-sidebar li").click(function(){//所有的li
		//设置单击的li选中
		$(this).siblings("li").removeClass("active");
		$(this).addClass("active");
		//更换正文
		var target = $(this).children("a").attr("name");
		$('#main').load(target);//加载点击目标.html中正文
		//为正文页面远程加载js
		load_script($(this));
	});
}
...
function load_script($li) {
	...
	if("user"==name){
		$.getScript("js/common/bootstrap-paginator.min.js");//动态加载分页框架
//		$.getScript("js/common/ajaxfileupload.js");//动态加载文件上传框架
	}
